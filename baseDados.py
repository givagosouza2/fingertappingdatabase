...
with tab1:
    ...
    if codigo_participante_especifico in participantes:
        ...
        with col3:
            ...
            ax.set_xlim(min(x_dados)-(max(x_dados) - min(x_dados))*0.25, max(x_dados)+(max(x_dados) - min(x_dados))*0.25)
            ax.set_ylim(min(y_dados)-(max(x_dados) - min(x_dados))*0.25, max(y_dados)+(max(x_dados) - min(x_dados))*0.25)
